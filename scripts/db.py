...
def cargar(nombre_archivo: str) -> Dict:
    ...
    if nombre_archivo == "personas.json":
        rows = query("""SELECT id, nombre, color, emoji, activo, telegram_chat_id,
                               push_subscriptions, pass_hash, orden, email, foto
    ...
    if nombre_archivo == "lista_mercado.json":
        rows = query("""SELECT id, item, cantidad, unidad, origen, categoria, comprado
    ...
    if nombre_archivo == "cursos.json":
        rows = query("""SELECT id, nombre, emoji, persona_id, lecciones_total,
                               lecciones_hechas, min_dia, recompensa, estado,
                               racha, mejor_racha, ultimo_estudio, historial, misiones
                        FROM cursos ORDER BY created_at""")
        return {"cursos": [_row_to_dict(r) for r in rows]}

    if nombre_archivo == "gastos.json":
        rows = query("""SELECT id, fecha, descripcion, monto, categoria,
                               pagado_por, participacion, tipo, created_at
                        FROM gastos ORDER BY fecha DESC, created_at DESC""")
        return {"gastos": [_row_to_dict(r) for r in rows]}
...
def guardar(nombre_archivo: str, datos: Dict) -> None:
    ...
    if nombre_archivo == "lista_mercado.json":
        _sync_lista("lista_mercado", datos.get("lista_mercado", []),
                    ["id","item","cantidad","unidad","origen","categoria","comprado"])
        return
    ...
    if nombre_archivo == "cursos.json":
        _sync_lista("cursos", datos.get("cursos", []),
                    ["id","nombre","emoji","persona_id","lecciones_total",
                     "lecciones_hechas","min_dia","recompensa","estado",
                     "racha","mejor_racha","ultimo_estudio","historial","misiones"],
                    json_cols={"historial","misiones"})
        return

    if nombre_archivo == "gastos.json":
        _sync_lista("gastos", datos.get("gastos", []),
                    ["id","fecha","descripcion","monto","categoria",
                     "pagado_por","participacion","tipo"])
        return
